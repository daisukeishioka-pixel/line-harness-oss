...
liffRoutes.get('/auth/line', (c) => {
  ...
  // /liff/* からのリダイレクトの場合はランディングページをスキップして直接OAuthへ
  if (redirect && redirect.startsWith('/liff/')) {
    return c.redirect(loginUrl.toString());
  }

  // それ以外（LP/SNS等）はランディングページを表示
...
liffRoutes.get('/auth/callback', async (c) => {
  ...
  try {
    ...
    if (redirect) {
      // /liff/* ページへのリダイレクトには friendId を付与
      const redirectUrl = new URL(redirect, baseUrl);
      if (redirectUrl.pathname.startsWith('/liff/')) {
        redirectUrl.searchParams.set('fid', friend.id);
      }

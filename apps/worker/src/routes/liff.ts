...
function liffInitScript(liffId: string, workersUrl: string): string {
  return `
    ...
    function initLiff() {
      // 1. URL の fid パラメータを優先（リッチメニュー postback 経由）
      ...
      // 2. LIFF SDK でログイン済みならプロフィール取得（login() は呼ばない）
    ...
    function _patchNavLinks() {
      ...
      for (var i = 0; i < links.length; i++) {
        ...
        if (href && href.indexOf('fid=') === -1) {
          links[i].setAttribute('href', href + (href.indexOf('?') >= 0 ? '&' : '?') + 'fid=' + encodeURIComponent(friendId));
        }
      }
    }
...
/**
 * GET /liff — ホーム画面へリダイレクト（LIFF SDK 不使用）
 * fid パラメータがあればそのまま引き継ぐ
 */
...
<body>
  ...
  <script>
    ...
    // fid パラメータまたは LIFF 経由で friendId を取得
    function _getQuery(name) {
      var m = location.search.match(new RegExp('[?&]' + name + '=([^&]*)'));
      return m ? decodeURIComponent(m[1]) : null;
    }
    ...
    function initSignup() {
      ...
      if (fid) {
        friendId = fid;
        return Promise.resolve({ success: true, data: { id: fid } });
      }
      return liff.init({ liffId: LIFF_ID }).then(function() {
        if (!liff.isLoggedIn()) { return Promise.reject('not_logged_in'); }
        return liff.getProfile();
      }).then(function(profile) {
        if (!profile) return Promise.reject('no profile');
    ...
    initSignup()
      ...
      .catch(function(err) {
        if (err === 'not_logged_in') {
          showError('このページはLINEトーク画面のメニューからアクセスしてください。');
        } else {
          console.error('Init error:', err);
          showError('初期化に失敗しました: ' + (err.message || err));
        }
      });
...
initLiff().then(function() {
  ...
  if (e === 'not_logged_in') {
    document.querySelector('.container').innerHTML = '<div class="card" style="text-align:center;padding:24px"><p style="font-size:14px;color:var(--text);margin-bottom:8px">ページを表示できません</p><p style="font-size:12px;color:var(--text-sub)">LINEトーク画面のメニューからアクセスしてください。</p></div>';
  } else if (e !== 'login') console.error(e);
});
...
initLiff().then(function() {
  ...
  if (e === 'not_logged_in') {
    document.getElementById('loading').innerHTML = '<div style="text-align:center;padding:24px"><p style="font-size:14px;color:#333;margin-bottom:8px">ページを表示できません</p><p style="font-size:12px;color:#888">LINEトーク画面のメニューからアクセスしてください。</p></div>';
  } else if (e !== 'login') { document.getElementById('loading').innerHTML = '<p class="empty">エラーが発生しました</p>'; console.error(e); }
});
...
// ─── マイページ エイリアス (/liff/membership → /liff/mypage) ─────
liffRoutes.get('/liff/membership', (c) => {
  const fid = c.req.query('fid') || '';
  const w = new URL(c.req.url).origin;
  const target = fid
    ? `${w}/liff/mypage?fid=${encodeURIComponent(fid)}`
    : `${w}/liff/mypage`;
  return c.redirect(target, 302);
});
